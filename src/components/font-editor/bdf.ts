...
export function parseBDF(text: string): Font {
    ...
    switch (keyword) {
      ...
      default:
        if (inProperties) {
          ...
          else if (keyword === "FONT_DESCENT") descent = parseNumbers(rest)[0] ?? 0;
  ...
  if (!box) {
    box = raws.reduce<Box>(
      (acc, item) => unionBox(acc, item.box),
      { w: 1, h: 1, ox: 0, oy: 0 },
    );
  }
...
export function serializeBDF(font: Font): string {
  ...
  out.push("COMMENT Generated by Empix Font Editor");
  ...
  const properties: [string, string][] = [
    ...font.properties,
    ["PIXEL_SIZE", String(Math.round((font.pointSize * font.resolutionY) / 72))],
