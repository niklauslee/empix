/**
 * Bitmap operations on an icon's pixel array. Every function returns a new
 * array — pixel arrays are treated as immutable so React and the undo stack can
 * compare them by reference.
 */

import { emptyPixels, getPixel, type Box } from "./icon";
...
function set(box: Box, pixels: boolean[], point: Point, on: boolean) {
  const { col, row } = point;
  if (col < 0 || col >= box.w || row < 0 || row >= box.h) return;
  pixels[row * box.w + col] = on;
}
...
export function drawRect(
  ...
  for (let row = top; row <= bottom; row++) {
    for (let col = left; col <= right; col++) {
      ...
      if (fill || edge) set(box, next, { col, row }, on);
    }
  }
  return next;
}
...
export function shift(
  ...
  for (let row = 0; row < box.h; row++) {
    for (let col = 0; col < box.w; col++) {
      if (getPixel(box, pixels, col - dcol, row - drow)) {
        next[row * box.w + col] = true;
      }
    }
  }
  return next;
...
export function flipHorizontal(box: Box, pixels: boolean[]): boolean[] {
  const next = emptyPixels(box);
  for (let row = 0; row < box.h; row++) {
    for (let col = 0; col < box.w; col++) {
...
export function flipVertical(box: Box, pixels: boolean[]): boolean[] {
  const next = emptyPixels(box);
  for (let row = 0; row < box.h; row++) {
    for (let col = 0; col < box.w; col++) {
...
export function invert(pixels: boolean[]): boolean[] {
  return pixels.map((on) => !on);
}

export function clear(box: Box): boolean[] {
