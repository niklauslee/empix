...
Local-first: no backend, no login — the scene is persisted in `localStorage`.
...
```
src/
  pages/            routes: index.astro (redirect → /screen), screen.astro, font.astro
  apps/
  ...
  lib/utils.ts      cn, detectPlatform, generateNewName, odd
