...
Local-first: no backend at all — the scene is persisted in `localStorage`.

Astro (fully static output) + React islands, deployed to Cloudflare Workers as
static assets.
...
```
src/
  pages/            routes: index.astro (redirect → /scene), scene.astro, font.astro
  apps/
...
Fully static: `astro build` emits static HTML/JS/CSS to `./dist`, deployed as
a Cloudflare Workers static-assets site (`wrangler.jsonc`, no `main` script,
no bindings). Live at <https://empix.niklauslee.workers.dev/>
